...
> **Nota de diseño:** toda la imagen de esta base usa modalidades **generables por 3D MedDiffusion** (CT y MRI de regiones soportadas: cabeza/cuello, tórax, abdomen). Por eso la cobertura evita mama y próstata (no soportadas por el modelo) y las reemplaza por patologías de órganos abdominales/torácicos equivalentes.
...
| Riñón | 2 (carcinoma de células renales, RCC temprano) | 3 (angiomiolipoma, quiste simple, pielonefritis) | 5 |
| Colon / GI | 3 (sigmoide, pólipo premaligno, neuroendocrino intestinal) | 2 (colitis ulcerosa, diverticulitis) | 5 |
| Hígado / vía biliar | 2 (HCC, colangiocarcinoma) | 2 (hemangioma, hiperplasia nodular focal) | 4 |
| Suprarrenal | 1 (carcinoma adrenocortical) | 1 (adenoma benigno) | 2 |
| Tiroides | 1 (carcinoma papilar) | 1 (nódulo benigno) | 2 |
| Linfoma | 2 (no Hodgkin, Hodgkin) | — | 2 |
| Estómago | 1 (adenocarcinoma infiltrante) | — | 1 |
...
```jsonc
{
  "gt_id": "GT-RENAL-001",                    // ID único, formato GT-<CÓDIGO>-<NÚMERO>
  "icd_10": "C64.9",                          // Código ICD-10 oficial
  "icd_10_description": "Neoplasia maligna del rinon...",
  "objective_data": {
  ...
  "radiologist_guidance": {
    "modality_priority": [ ... ],             // ej: ["abdominal_CT", "abdominal_MRI"] (solo CT/MRI soportado por MedDiffusion)
    "views_recommended": [ ... ],
    "imaging_location": {
      "body_region": "...",
...
```jsonc
{
  ...
  "current_symptoms": [
    ...
    "dolor en flanco derecho 3 semanas"
  ],
  "medical_history": [
    { "date": "2021-03", "event": "Hipertension arterial en tratamiento." },
    { "date": "2023-11", "event": "Ecografia abdominal: lesion renal derecha a caracterizar." }
  ],
  "current_labs": {
    "LDH": 320,
    "calcio_serico": 10.8,
...
{
  "case_id": "case_001",
  "correct_gt_ids": ["GT-RENAL-001"],                     // GT(s) que el sistema DEBE matchear
  "acceptable_secondary_gt_ids": ["GT-ANGIOMIOLIPOMA-001"], // GTs aceptables como secundarios
  ...
  "notes": "Carcinoma de celulas renales clasico con hematuria en paciente de riesgo."
}
```
...
| **FP** (false positive borderline) | 15 | Síntomas sospechosos pero benigno (pielonefritis, neumonía, diverticulitis, hemangioma, hiperplasia nodular focal, adenoma suprarrenal) — el sistema debe NO sobre-derivar |
| **FN** (false negative sutil) | 15 | Cáncer atípico (gástrico infiltrante sin masa clara, pulmón en no fumador, páncreas con diabetes nueva, masa renal pequeña incidental) — el sistema debe alertar pese a perfil atípico |
...
1. **Contenido sintético:** los casos y la base GT están anclados en guías clínicas públicas (NCCN, ESMO, ICD-10 WHO, LI-RADS, Lung-RADS, Bosniak) pero no fueron revisados por especialistas certificados. **No usar para decisiones clínicas reales.**

2. **Sin pediatría ni embarazo:** el dataset cubre adultos (22-79 años) y no incluye condiciones gestacionales ni pediátricas.

3. **Distribución de GTs no uniforme:** ca colorrectal, renal y pulmón están sobre-representados — refleja la prevalencia clínica real y las regiones cubiertas por MedDiffusion.

4. **Casos de borde intencionales:** algunos casos referencian dominios oncológicos sin GT exacto en la base (ej. `case_109` masa retroperitoneal de origen incierto). Son intencionales para evaluar cómo el sistema responde cuando no hay match perfecto.
