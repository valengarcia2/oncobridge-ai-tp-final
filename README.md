...
- Entregar un sistema que cualquier persona con Python pueda instalar y correr desde cero.
...
**Usamos salida estructurada (Structured Outputs) en cada llamada al modelo en vez de parsear texto libre.** Le pasamos a la API un schema de Pydantic y la API garantiza que la respuesta lo cumple exactamente. Esto nos deja construir el resto del pipeline sobre un contrato de datos confiable.
...
- **Una imagen de referencia ilustrativa**, generada con Stable Diffusion (`Nihirc/Prompt2MedImage`) a partir del prompt ya definido en el ground truth para esa condición. Se cachea por diagnóstico (`gt_id`), no por paciente. Si dos pacientes distintos matchean la misma hipótesis principal, la imagen no se regenera.
...
| `LLM_THINKING_BUDGET` (effort) configurable | Poder ajustar cuánto "piensa" el modelo antes de responder |
| Imágenes por `gt_id` | Evitar regenerar la misma imagen si distintos pacientes matchean el mismo diagnóstico |
...
Componente 2 recibe el output de Componente 1 como input. Este comando corre C1 internamente para producir ese input, e imprime el output de C2:
...
Corre el sistema real con una interfaz gráfica, pensada para una audiencia no técnica:
